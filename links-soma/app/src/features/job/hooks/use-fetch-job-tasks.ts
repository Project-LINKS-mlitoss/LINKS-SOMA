...
const transformJobTasks = (data: SelectJobTask[]): SelectJobTask[] => {
  const result = data;
...
const fetcher = async ([jobId]: [number]): Promise<SelectJobTask[]> => {
  const result = await window.ipcRenderer.invoke("selectJobTasks", jobId);
  return transformJobTasks(result);
};

export const useFetchJobTasks = ({
  jobId,
}: Params): SWRResponse<SelectJobTask[]> => {
  const swr = useSWR([jobId, useFetchJobTasks.name], ([args]) =>
    fetcher([args]),
  );
  return swr;
};

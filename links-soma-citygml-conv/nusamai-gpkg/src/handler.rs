...
impl GpkgHandler {
    ...
    pub async fn connect_url(str: &str) -> Result<Self, GpkgError> {
        Self::connect(SqliteConnectOptions::from_str(str)?).await
    }

    async fn connect(conn_opts: SqliteConnectOptions) -> Result<Self, GpkgError> {
        let conn_opts = conn_opts
            .create_if_missing(true)
            .synchronous(SqliteSynchronous::Normal)
            .journal_mode(SqliteJournalMode::Wal);
...
impl<'c> GpkgTransaction<'c> {
    ...
    pub async fn get_columns(&mut self, table_name: &str) -> Result<HashSet<String>, GpkgError> {
        let executor: &mut SqliteConnection = self.tx.acquire().await.unwrap();
        let query_string = format!("PRAGMA table_info(\"{}\");", table_name);

        let rows = sqlx::query(&query_string).fetch_all(&mut *executor).await?;
    ...
    pub async fn alter_table_add_columns(
        ...
        for column in missing_columns {
            let alter_query = format!(r#"ALTER TABLE "{}" ADD COLUMN "{}" TEXT;"#, table_name, column);

            if let Err(e) = sqlx::query(&alter_query).execute(&mut *executor).await {
                eprintln!("Table {} Failed to add column {}: {}", table_name, column, e);
                continue;
            }

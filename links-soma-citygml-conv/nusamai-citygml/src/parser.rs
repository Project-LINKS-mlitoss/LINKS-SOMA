...
impl<'b, R: BufRead> SubTreeReader<'_, 'b, R> {
    ...
    pub fn find_uom_attribute(&mut self) -> Option<String> {
        let Some(start) = &self.state.current_start else {
            panic!("find_uom_attribute() must be called immediately after encountering a start tag.");
        };
        for attr in start.attributes().flatten() {
            if attr.key.as_ref() == b"uom" {

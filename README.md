...
本システムは、住民基本台帳やPLATEAU データ、本システムは、水道情報や住民基本台帳、PLATEAU データなど自治体が保有する既存データを活用し、地域ごとに最適化した空き家推定AIモデルを用いて、建物が空き家と思われる確率を推定するデスクトップアプリケーションです。 

- 地域の空き家確率を推定
  - 自治体が実施した空き家調査結果や把握済みの空き家台帳などを教師データにし、各自治体の保有データをもとに「その建物が空き家である確率」を推定します。
  - AIの学習に十分な教師データを有していない場合には、学習済みのプリセットモデルを利用することができます。
...
- **名寄せ処理** 
  - 水道開閉栓状況・使用量、住民基本台帳、登記情報、PLATEAU（または家屋現況図）などを自動処理し、ひとつのデータに統合

- **AI モデル構築**

  - 自治体が実施した空き家調査結果や把握済みの空き家台帳などを教師データとして学習し、空き家推定AIモデルを自動生成
  - 上位K件の適合率（Precision@K）とリフト値を自動計算し、精度を把握

- **空き家確率の推定**

  - 構築した AI モデルやプリセットモデルを用い、推定したい時点のデータから建物ごとに空き家確率を推定
...
![image](./docs/images/feature-1.png)
...
- 任意で利用可能な追加データ
    - 登記情報
    - 建物ポリゴン
        - PLATEAU の CityGML → CityGML Converter で gpkg に変換、または家屋現況図（Shapefile 形式やジオパッケージ）
    - 推定対象選定用データ（建物種別がわかるデータ）
    - ジオコーディング済みデータ（住所と緯度経度の対応）
    - 地域集計用ポリゴン（自治体独自の地区、学区など）

![image](./docs/images/feature-2.png)
...
サンプルデータは[こちら](https://github.com/Project-LINKS-mlitoss/LINKS-SOMA/tree/main/example/data)からダウンロードしてください。
...
LINKS-SOMA/
├── docs/
│ ├── 【公開版】LINKS_SOMA_空き家推定システムマニュアル.pdf
│ └── (その他のドキュメント)
├── examples/
│ └── data/
│   └── (サンプルデータ、テンプレートデータ)
...
- 本システムの活用のご相談や、技術についてのご質問：株式会社ユーカリヤ([info@eukarya.io](mailto:info@eukarya.io))
